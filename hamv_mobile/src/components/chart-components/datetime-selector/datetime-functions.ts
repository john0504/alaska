...
export const DT_DAY = (marker: Date, direction: number) => {
  ...
  const rangeText = moment(startTime).format('MMMM DD');
  const preRangeText = moment(startTime - 24 * 60 * 60).format('MMM DD');
  const postRangeText = moment(startTime + 24 * 60 * 60).format('MMM DD');
...
export const DT_MONTH = (marker: Date, direction: number) => {
  ...
  const endTime = marker.getTime() - 1000;
  const postRangeText = moment(marker.getTime()).format('MMM');
  const pastMonth = (m + 11) % 12;
  marker.setMonth(pastMonth);
  const preRangeText = moment(marker.getTime()).format('MMM');
  // const rangeText = moment(startTime).format('MMM YYYY');
  const rangeText = moment(startTime).format('MMMM');
...
export const DT_YEAR = (marker: Date, direction: number) => {
  ...
  const preRangeText = moment(startTime - 365 * 24 * 60 * 60).format('YYYY');
  const postRangeText = moment(startTime + 365 * 24 * 60 * 60).format('YYYY');
