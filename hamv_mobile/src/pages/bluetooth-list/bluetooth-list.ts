...
export class BluetoothListPage {
  ...
  constructor(
    ...
    this.subs = [];
    this.bluetoothList = [];
    // this.bluetoothList = [{ name: "ALASKA-01234", id: 1 }, { name: "CAR BT-123456", id: 2 }];
  ...
  private startScanning() {
    ...
    this.ble.startScan([]).subscribe(device => {
      if (this.checkDeviceName(device.name, device.id)) {
        ...
        this.printLog(device.name, "id", device.id);
        if (this.scanTime == 0) {
  ...
  connectToDevice(device) {
    if (!device.checked) {
      this.bluetoothList.forEach(bledevice => {
        bledevice.checked = false;
      });
    }
  ...
  closePage() {
    if (!this.isClosePage) {
      this.navController.push('BluetoothDevicePage').then(() => {
