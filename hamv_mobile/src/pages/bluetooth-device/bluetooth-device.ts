...
export class BluetoothDevicePage {
  ...
  ionViewDidLoad() {
    ...
    this.cancelLoading = setInterval(() => {
      clearInterval(this.cancelLoading);
      this.status = this.STATUS_NO_DEVICE;
      this.closePage();
    }, 10000);
  }
  ...
  ionViewWillLeave() {
    ...
    this.subs.length = 0;
    var msg = this.translate.instant('BLUETOOTH.BLUETOOTH_LEAVE');
    if (this.status == this.STATUS_NO_DEVICE) {
      msg = "找不到裝置";
    }
  ...
  private loadStorage() {
    ...
    this.storage.get('connectDevices').then((connectDevices) => {
      if (connectDevices) {
        ...
        this.linkDevice(this.bleList[0]);
            ...
            switch (jsonObj.response) {
              case "info":
                ...
                if (deviceItem._device.profile.esh.model == "") {
                  ...
                  this.cd.detectChanges();
                  this.bleList.forEach(device => {
                    if (device.device == deviceItem.deviceId) {
                      device.model = deviceItem._device.profile.esh.model;
                      device.service = deviceItem.serviceId;
                      device.characteristic = deviceItem.characteristicId;
                    }
                  });
                  this.storage.set('connectDevices', this.bleList);
                }
                break;
            }
